(function ($) {
    $.fn.extend({
        dataTable: function (o) {
            ...
            var B = A.attr('id');
            A.addClass('data-table')
                .append("<div class=\"data-table-header\" id=\"" + B + "_header\">" +
                "<div class=\"" + (options.selectable ? "container-actions-selectable" : "container-actions")  + " \" id=\"" + B + "_container_actions\"></div>" +
                "<div class=\"container-filters\" id=\"" + B + "_container_filters\"></div>" +
                "</div>" +
                "<div class=\"data-table-body\">" +
                "<table id=\"" + B + "_table_view\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" width=\"100%\"></table>" +
                "</div>" +
                "<div class=\"data-table-footer\">" +
                "<div class=\"container-paging\" id=\"" + B + "_container_paging\">" +
                "<a href=\"javascript:;\" id=\"" + B + "_button_previous\" class=\"data-table-button-previous btn btn-default button-white\" title=\"" + options.messages.previousPage + "\"><i class=\"fa fa-caret-right\"></i></a>" +
                "<a href=\"javascript:;\" id=\"" + B + "_button_next\" class=\"data-table-button-next btn btn-default button-white\" title=\"" + options.messages.nextPage + "\"><i class=\"fa fa-caret-left\"></i></a>" +
                "</div>" +
                "<div class=\"container-results\" id=\"" + B + "_container_results\">" +
                "<div class=\"cell\">" +
                "<div id=\"" + B + "_label_results\"></div>" +
                "</div>" +
                "</div>" +
                "<div class=\"container-show-rows\" id=\"" + B + "_container_show_rows\">" +
                "<div class=\"cell\"><span class=\"label-field\">" + options.messages.showRows + "</span></div>" +
                "<div class=\"cell select-field\">" +
                "<select id=\"" + B + "_select_show_rows\" class=\"form-control\"></select>" +
                "</div>" +
                "</div>" +
                "<div class=\"container-goto\" id=\"" + B + "_container_goto\">" +
                "<div class=\"cell button-field\">" +
                "<a id=\"" + B + "_button_goto_page\" title=\"Go\" class=\"button-white\"><span>Go</span></a>" +
                "</div>" +
                "<div class=\"cell\">" +
                "<input id=\"" + B + "_input_goto_page\" type=\"text\" autocomplete=\"off\" class=\"goto\" />" +
                "</div>" +
                "<div class=\"cell\"><span class=\"label-field\">Go to:</span></div>" +
                "</div>" +
                "</div>");
                    ...
                    if (h < options.pagingStart) {
                        ...
                        if(options.groupBy != null) {
                            ...
                            for (var x = 0; x < options.tableColumns.length; x++) {
                                ...
                                if(options.tableColumns[x].editable != undefined
                                    && options.tableColumns[x].editable == true) {
                            ...
                            /*
                             if(hasEditable) {
                             var editableActions = $("<div class=\"editable-actions\">" +
                             "<div class=\"handle\"><a class=\"move-row\">Move</a></div>" +
                             "<div class=\"insert\"><a class=\"add-row\"><i class=\"fa fa-plus\"></i></a></div>" +
                             "<div class=\"delete\" style=\"margin-right: " + (A.width() - 1) + "px\"><a class=\"delete-row\"><i class=\"fa fa-close\"></i></a></div>" +
                             "</div>").appendTo(r.find("td.first-item"));

                             // add row
                             editableActions.find("a.add-row").click(function(e) {

                             var $this = $(this);
                             $this.closest("tr").after("<tr><td>1</td></tr>");

                             });

                             // delete row
                             editableActions.find("a.delete-row").click(function(e) {
                    ...
                    if(options.groupBy != null) {
                        ...
                        if (options.tableColumns[i].sortable) {
                            ...
                            if(options.sort != undefined) {
                                if(options.sort.key != undefined
                                    && options.sort.order != undefined) {
                                    if(options.sort.key == h.attr("key")) {

                                        //console.log("sort " + options.sort.key + " _____" + h.attr("key"));

                                        h.attr("asc", (options.sort.order == "asc"));
                                        h.trigger('click');
